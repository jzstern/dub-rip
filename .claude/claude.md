...
- **`player_client` follows yt-dlp's `default` chain** (`visionos,web` on 2026.08.19) — do not hand-pick a list. It was pinned to `web_safari,mweb,tv` until 2026-09-14, when YouTube began bot-checking all three from Railway: every download failed with "Sign in to confirm you're not a bot" while bgutil-pot minted valid tokens for each attempt, on a release unchanged for 41 days. A hand-picked list is frozen at the moment someone picked it; the default moves with upstream as YouTube shifts. `visionos` takes no PO token at all (`VISIONOS` is absent from `WEBPO_CLIENTS` in `youtube/pot/utils.py`), so most downloads never touch bgutil-pot — `web` is the fallback that does. The pin existed to stop a non-WebPO lead client's formats winning `bestaudio` and then 403ing; that client was `android_vr`, and the hazard did not reproduce with `visionos` (format 251 downloaded cleanly from a Railway PR env on 2026-09-16). If it ever does, exclude that one client with `player_client=default,-visionos` rather than restoring a list. Re-read `_DEFAULT_CLIENTS` in `yt_dlp/extractor/youtube/_video.py` when bumping the pin.
