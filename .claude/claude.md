...
- **Runbook**, keyed off the `stage` a failed run reports
  ...
  | `page_rate_limited` | YouTube 429s the watch page from the egress IP (`Unable to download webpage: HTTP Error 429`). yt-dlp doesn't retry it and has no visitor data afterwards, so the run's ERROR line is typically a bot-check (in every observed run) — but the first failure is IP throttling, not a client-list problem | A single 429 can be a transient burst (bursts get the IP throttled for minutes), so confirm it persists across consecutive canary runs first. If it does, rotate the Railway Static Outbound IP, or move to a residential proxy — see [`docs/deployment-strategy.md`](../docs/deployment-strategy.md). Don't touch the client list |
