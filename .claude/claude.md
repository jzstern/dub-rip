...
**Production egresses through Railway Static Outbound IPs (enabled 2026-09-17).** The previous egress IP was refused by YouTube for every media fetch — DASH formats 403'd, HLS fragments 403'd then 401'd — and no client or format change fixed it; switching IPs did, immediately. This is dashboard state with no trace in the repo, so check it with `railway outbound-network status --service dub-rip --environment production --json` rather than assuming. The setting is high-availability (`staticIp.highAvailability: true`) with **three** addresses, not one — on 2026-09-18 they were 162.220.234.241 (zone eqdc4a) and 152.55.180.240 / 152.55.180.241 (zone eqdc16a) — so "the egress IP" here means whichever a request left from, and **which of them is flagged is unknown**. Railway does not guarantee the addresses are dedicated, and sustained traffic can get them flagged too. On 2026-09-18 two different failures ended in the same bot check: a cold bgutil-pot sidecar (`Error reaching …/ping` in stderr), and a case with tokens minted and no ping warning where a matched request to a PR env succeeded at the same instant, which points at production's egress IP (one pair; which of the three addresses, or how many, is unknown). The watch-page `HTTP Error 429` is chronic from production's egress since the static IPs went in and also appears in passing runs, so it is not what distinguishes a failure. The durable fallback is a residential proxy; neither that nor re-provisioning the addresses has been done. See [`docs/deployment-strategy.md`](../docs/deployment-strategy.md#symptom-bot-check-with-a-watch-page-http-error-429-in-stderr-two-failure-modes-2026-09-18).
...
- **Runbook**, keyed off the `stage` a failed run reports
  ...
  | `page_rate_limited` | The watch page 429'd (`Unable to download webpage: HTTP Error 429`) and the run then failed with the bot check. The 429 is **chronic** from production's egress and also appears in passing runs (the 19:43 UTC manual canary on 2026-09-18), so the label does not say why the run failed — two different failures carry it | Read the run's `detail` and logs for `[pot:bgutil:http] Error reaching …/ping`. **Present:** a cold bgutil-pot sidecar, so no player PO token — should now be rare from the canary, which waits for `/ping` first (look for `[canary] bgutil-pot answered /ping after …`); the app's in-process retry recovered the one observed case (a direct `curl` of the download route with no preview, 2026-09-17 00:59:41 UTC). **Absent, with `Generating a player PO Token …` lines:** the IP-level case — confirm the bot check persists across consecutive canary runs, then see the IP options in [`docs/deployment-strategy.md`](../docs/deployment-strategy.md#symptom-bot-check-with-a-watch-page-http-error-429-in-stderr-two-failure-modes-2026-09-18). Don't touch the client list |
  | `player_bot_check` | A bot-check with no watch-page 429 (a 429 is classified as `page_rate_limited` above): the burned-client-list signature of 2026-09-14 — but an IP-level block can look the same, so check for tokens minted and no `Error reaching …/ping` before changing the client list | Check `_DEFAULT_CLIENTS` in yt-dlp's `_video.py` against `YOUTUBE_EXTRACTOR_ARG` — don't hand-pick a list (see the `player_client` history in `yt-dlp-binary.ts`) |
...
- **The webpage client does not announce itself when the watch page works, and its silence is not a fault.** yt-dlp reads one client's player response straight out of the watch page rather than making a separate innertube call — whichever client `_DEFAULT_WEBPAGE_CLIENT` in `_video.py` names (`web` on 2026.08.19, `web_safari` on 2026.07.04). That client prints no `Downloading … client config` / `… player API JSON` lines, and takes no *player* PO token, because it issues no player request to attach one to. With a usable watch page, a healthy 2026.08.19 log reads `Downloading webpage` → `Downloading visionos player API JSON`, with nothing for `web` — that is what a dev box shows. **Production logs on the static IPs mostly do not look like that.** Since the static IPs went in, 4 of the 5 runs inspected before 2026-09-18 (deployments 94d4b6b9 — created 2026-09-17 00:58 UTC, presumably the redeploy after enabling them — and 90aee016) and every run inspected on 2026-09-18 also log `Downloading web player API JSON`; on the previous egress IP, none of the runs inspected did (small samples; the run-by-run detail is in [`docs/deployment-strategy.md`](../docs/deployment-strategy.md#symptom-bot-check-with-a-watch-page-http-error-429-in-stderr-two-failure-modes-2026-09-18)). Per yt-dlp source that line appears when the watch page's own player response is unavailable (the `Downloading iframe API JS` and `Downloading player <id>-main` lines beside it also appear on healthy runs, so they mark nothing), so — *inference, not measured* — from the static addresses the watch page is frequently or always unusable and `web` routinely makes an explicit player call there; that this began with the static IPs rather than being new on 2026-09-18 is likewise inference. A watch-page `HTTP Error 429` (with `Missing required Visitor Data`) is also seen on a passing run — the 19:43 UTC manual canary on 2026-09-18 — so it is chronic, not a failure marker. Read a production log against that, not against the dev-box one. Force the explicit call with `--extractor-args "youtube:player_skip=webpage"` to see which path a release takes.
- **`fetch_pot=always` is not optional, and it is not a tuning knob.** Under yt-dlp's default `fetch_pot=auto`, a PO token is minted only when the *client's own policy* marks one required or recommended — and `web` (like `web_safari`, via the shared `WEB_PO_TOKEN_POLICIES`) declares `PlayerPoTokenPolicy(required=False)`. So whenever `web` issues a player request (common on the static IPs — see the webpage-client bullet above), that innertube **player** request goes out with no token, YouTube bot-checks it from Railway's datacenter IP, and it fails with "Sign in to confirm you're not a bot" *while bgutil-pot sits there healthy and is never asked for anything*. It is inert for `visionos`, which takes no WebPO token either way — so a missing `Generating a player PO Token …` line is normal only when no `Downloading web player API JSON` line appears either, and a fault once `web` has issued a player request. `fetch_pot=always` only makes yt-dlp ask: when the sidecar is cold the plugin's `/ping` check fails (`[pot:bgutil:http] Error reaching …/ping`) and the player request is token-less anyway — mode (i) in the [2026-09-18 write-up](../docs/deployment-strategy.md#symptom-bot-check-with-a-watch-page-http-error-429-in-stderr-two-failure-modes-2026-09-18), which the app's in-process retry recovered in the one observed case (a direct `curl` of the download route with no preview). Both arg builders share `YOUTUBE_EXTRACTOR_ARG` in `src/lib/yt-dlp-binary.ts` so the two halves can't drift apart — add new call sites through that constant, never by writing the string out again.
...
- **`POST /api/preview` fire-and-forget pings `${BGUTIL_POT_URL}/ping`** to wake the sleeping sidecar while the user is still reading the preview. Use `/ping` (the healthcheck), never `/get_pot` — the latter does real BotGuard work speculatively. Never await it, always bound it with `AbortSignal.timeout`, always swallow the rejection. That ping alone is not enough: it is fire-and-forget, so a click before the sidecar is listening (up to ~9 s by Railway's log timestamps; the one direct measurement was 1.3 s) still reaches yt-dlp while it boots, and `GET /api/download-stream` also *awaits* `waitForBgutilPot` (12 s cap, stops on client disconnect, never fails the download) before it starts any yt-dlp work — see [`docs/deployment-strategy.md`](../docs/deployment-strategy.md#the-download-waits-for-the-sidecar). Don't remove that wait to "save a round trip": without it a request that reaches yt-dlp before the sidecar listens is bot-checked and only the retry recovers it. (In the browser flow the first contact is `/api/preview/details`, which does not wait.)
