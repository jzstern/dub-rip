...
The prewarm is fire-and-forget, so a request that reaches yt-dlp before the sidecar is listening still meets a booting sidecar. yt-dlp's bgutil plugin checks `/ping` once, caches a failure for 60 s, and then has no PO token for the run, so YouTube bot-checks the `web` player request — the same message a throttled IP produces. Until this wait existed, the app's retry was the only thing recovering that case.

Two things about it are not well known. **How long the window is:** Railway's log timestamps put "Started POT server" up to ~9 s after "Starting Container", but those lines are flushed in batches, so that is an upper bound; the one direct measurement (the 2026-09-18 19:43 UTC canary's `/ping`, which answered after 1.3 s) was much shorter. **Who hits it:** the one observed case (2026-09-17 00:59:41 UTC — a batched log timestamp; the request began about 00:59:33) was a direct `GET /api/download-stream` from `curl` with no preview beforehand, not a browser session, so it shows the mechanism, not that browser users reach it. In the browser flow a paste calls `/api/preview` (which prewarms) and then `/api/preview/details` — an unwaited yt-dlp extraction that starts within about a second of the prewarm, so that is where a cold sidecar is most likely to be met first. The download route meets it only if the user skips the preview, clicks before it finishes, or the cached details (10-minute TTL) have expired. The `[canary] bgutil-pot answered /ping after N attempt(s), Xms` line, and the `bgutil-pot answered /ping after …` line the download route logs when it had to wait, will give real numbers.

`GET /api/download-stream` therefore calls `waitForBgutilPot` (`src/lib/wait-for-bgutil-pot.ts`, shared with the canary) right after its first status event and before any yt-dlp work, so the oEmbed lookup, the download, and any details extraction that is not already cached run against a sidecar that has answered `/ping`. (In the browser flow the details are normally cached from the paste-time call, which did not wait.) When the sidecar is warm that is one round trip. When it is cold the client sees `Waking up the downloader...` instead of a silent stall. The wait polls every 500 ms and stops the moment the client disconnects. It starts no new attempt once about 12 s have passed, but an attempt already in flight can add up to its 3 s timeout, so the hard bound is roughly 13.5 s.

It never fails the download. If the sidecar stays silent for the whole wait the attempt goes ahead anyway, the retry loop is still there as the safety net, and a `warning` breadcrumb records the attempts and time so a failure that follows can be read alongside it. The worst case is a sidecar that is down: the user waits 12–13.5 s before the failure they were going to get, where before they waited a few seconds. That is the cost of not guessing whether a slow sidecar is starting or dead.

`/api/preview/details` (the metadata extraction that runs when a URL is pasted) does not wait; it goes through its own retry loop and a failure there costs a missing duration and a Sentry event, not a download. In the browser flow it is the first extraction to meet a cold sidecar (see above), so it is the higher-value place to give the same wait; that is not done here.
...
- A direct `GET /api/download-stream` from `curl` (no preview beforehand) at 2026-09-17 00:59:41 UTC (a batched log timestamp; the request began about 00:59:33): the first attempt failed with the 429, the `/ping` `TransportError`, `Missing required Visitor Data` and the bot check; the app's in-process retry logged `[pot:bgutil:http] Generating a player PO Token for web client via bgutil HTTP server` and succeeded, and the MP3 was delivered. **The retry masked this mode for that request.** That it also masks it for browser users is not shown: after a paste the page's first contact with the sidecar is `/api/preview/details`, not the download route.
...
Real users were only partly covered: `POST /api/preview` fires a fire-and-forget `/ping`, and a click before the sidecar is listening could still hit it. `GET /api/download-stream` now waits for the sidecar too (see [The download waits for the sidecar](#the-download-waits-for-the-sidecar)).
